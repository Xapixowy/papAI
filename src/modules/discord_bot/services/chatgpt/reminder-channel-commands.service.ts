...
export class ReminderChannelCommandsService {
  ...
  public async reminderChannelAddHandler({
    ...
    if (
      ...
      if (newReminderChannelsSetting.isErr()) {
        return this.generateSimpleEmbed({
          description: ErrorCodeMessageMap[newReminderChannelsSetting.error],
          variant: 'error',
        });
      }
    ...
    if (existingReminderChannels.isErr()) {
      return this.generateSimpleEmbed({
        description: ErrorCodeMessageMap[existingReminderChannels.error],
        variant: 'error',
      });
    }
    ...
    if (newChannel.isErr()) {
      return this.generateSimpleEmbed({
        description: ErrorCodeMessageMap[newChannel.error],
        variant: 'error',
      });
    }
